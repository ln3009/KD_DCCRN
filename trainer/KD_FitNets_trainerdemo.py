...
class FitNets_Trainer(BaseTrainer):
    ...
    def __init__(self, config, teacher_model, model, train_iter, valid_iter, device="cpu"):
        ...
        # reconfig path
        self.checkpoints_path = os.path.join(self.base_path, "checkpoints", "fitnets_x1_16demo2")
        self.logs_path = os.path.join(self.base_path, "logs", "train", "fitnets_x1_16demo2")
        # mkdir path
        ...
        self.monitor_layer_idx = int(mon_cfg.get("layer_idx", 0))      # 监控第几层（默认 0）
...
if __name__ == "__main__":
    '''
    运行示例：
    python trainer/KD_FitNets_trainerdemo.py -TC config/base_config.toml -SC config/KD_FitNets_config.toml --gpu 1

    TensorBoard：
    tensorboard --logdir logs/train/fitnets_x1_4_1_2
    '''
