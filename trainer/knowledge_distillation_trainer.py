...
class KnowledgeDistillationTrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch):
        loss_total = 0.0
        teacher_loss_total = 0.0   # 累加教师 loss
        student_loss_total = 0.0   # 累加学生 loss
        ...
        for noisy, clean in tqdm(self.train_iter, desc="train"):
            ...
            student_loss = self.loss(enh, clean)
            ...
            teacher_loss_total += teacher_loss.item()              # 新增
            student_loss_total += student_loss.item()              # 新增
            loss_total += loss.item()

        avg_teacher = teacher_loss_total / len(self.train_iter)
        avg_student = student_loss_total / len(self.train_iter)
        
        print(f"[Epoch {epoch}]  Teacher loss: {avg_teacher:.4f}  |  Student loss: {avg_student:.4f}")
        self.update_scheduler(avg_student)

        self.writer.add_scalar("loss/teacher", avg_teacher, epoch)
...
if __name__ == "__main__":
    '''
    python trainer/knowledge_distillation_trainer.py   -TC config/base_config.toml   -SC config/lite_v1_config.toml
    '''
